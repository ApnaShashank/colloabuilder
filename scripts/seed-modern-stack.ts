...
async function seedModernStack() {
    try {
        await connectDB();
        console.log("Connected to database for seeding modern stack...");

        // Check if data already exists to avoid duplicates (optional)
        // Or just clear and re-seed
        // await Lesson.deleteMany({ category: { $in: ["react", "nextjs", "tailwind"] } });
