...
const DEPLOY_STATUS = {
  ...
  building: { label: "Building",     color: "text-primary/70",   dot: "bg-primary",     icon: Loading01Icon },
...
export default function ProjectsPage() {
  ...
  const stats = [
    ...
    { label: "Live Deploys",   val: projects.filter((p) => p.deployStatus === "live").length.toString(),     icon: GlobalIcon },
  ];
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <motion.div {...fadeUp} transition={{ duration: 0.5 }} className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div>
            ...
            <h1 className="font-headline font-black text-4xl text-white tracking-tight italic uppercase">Project <span className="text-primary opacity-40">Hub</span></h1>
          ...
          <div className="flex items-center gap-4">
            ...
            <motion.button
              ...
              className="bg-primary hover:bg-neutral-200 text-black px-6 py-3 rounded-xl font-black text-xs uppercase tracking-widest flex items-center gap-2 border border-white/10 shadow-lg transition-all italic"
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-10">
          {stats.map(({ label, val, icon: Icon }, i) => (
            ...
            >
              ...
              <div>
                {loading ? (
                  <SkeletonComponent className="w-12 h-8 mb-1" />
                ) : (
            ...
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="bg-[#0e0e0e] border border-white/[0.06] rounded-2xl p-6 space-y-4">
                <div className="flex justify-between items-start">
                  <div className="flex gap-4">
                    ...
                    <div className="space-y-2">
                      <SkeletonComponent className="w-40 h-5" />
                      <SkeletonComponent className="w-24 h-4" />
                    </div>
                  </div>
                ...
                <div className="pt-4 border-t border-white/[0.04] flex justify-between">
                  <SkeletonComponent className="w-20 h-4" />
                  <SkeletonComponent className="w-24 h-8 rounded-lg" />
                </div>
              </div>
            ))}
          </div>
        ) : filteredProjects.length === 0 ? (
          <div className="py-20 text-center bg-[#0e0e0e] border border-white/[0.04] rounded-[2.5rem]">
            ...
            {!searchQuery && (
              <button 
                onClick={() => setIsCreateModalOpen(true)}
                className="bg-white/[0.03] border border-white/[0.08] text-white px-6 py-2.5 rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-white/[0.05] transition-all italic"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {filteredProjects.map((proj, i) => (
              <motion.div
                ...
                className="bg-[#0e0e0e] border border-white/[0.06] rounded-2xl p-6 group hover:border-primary/20 transition-all"
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                       ...
                       className="w-8 h-8 rounded-lg bg-white/[0.03] border border-white/[0.08] flex items-center justify-center text-neutral-500 opacity-0 group-hover:opacity-100 transition-all hover:bg-red-500/10 hover:text-red-500 hover:border-red-500/20"
                    >
              ...
              <p className="text-neutral-500 text-xs leading-relaxed mb-5 line-clamp-2">
                ...
                {(() => {
                  ...
                  return (
                    <div className={`flex items-center gap-2 mb-4 px-3 py-1.5 rounded-lg ${
                      ...
                      ds === "building" ? "bg-primary/5 border border-primary/10" :
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Link
                      to={`/dashboard/projects/${proj._id}/edit`}
                      className="flex items-center gap-1 text-[10px] text-neutral-700 hover:text-[#6366f1] transition-colors font-bold uppercase tracking-widest px-2 py-1 rounded-lg hover:bg-[#6366f1]/10 border border-transparent hover:border-[#6366f1]/20"
                    >
                      <Code className="h-3 w-3" />
                      Edit Code
                    </Link>
                    <Link to={`/dashboard/projects/${proj._id}`} className="flex items-center gap-1 text-[11px] text-neutral-600 group-hover:text-primary transition-colors font-bold uppercase tracking-widest">
                      <span>Open</span>
                      <HugeiconsIcon icon={ArrowRight01Icon} size={12} className="ml-1 group-hover:translate-x-0.5 transition-transform" variant="stroke" strokeWidth={1.5} />
                    </Link>
                  </div>
                </div>
              </motion.div>
        ...
        {isCreateModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 lg:p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl bg-[#0e0e0e] border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden"
            >
              <div className="p-8 lg:p-12">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h2 className="text-2xl font-headline font-black text-white uppercase tracking-tight">Create New Project</h2>
                    <p className="text-neutral-500 text-[10px] font-bold uppercase tracking-widest mt-1">Start building your next idea.</p>
                  </div>
                  <button 
                    onClick={() => setIsCreateModalOpen(false)}
                ...
                <form onSubmit={handleCreate} className="space-y-6">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] font-black text-neutral-600 uppercase tracking-widest ml-1">Project Description</label>
                    <textarea 
                      rows={3}
                  ...
                  <div className="grid grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] font-black text-neutral-600 uppercase tracking-widest ml-1">Visibility</label>
                      <div className="flex bg-white/[0.02] border border-white/[0.06] rounded-xl p-1">
                        <button 
                          type="button"
                          onClick={() => setFormData({ ...formData, isPublic: true })}
                          className={`flex-1 py-3 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${formData.isPublic ? 'bg-primary text-black' : 'text-neutral-600'}`}
                        >
                          Public
                        </button>
                        <button 
                  ...
                  <div className="pt-4 flex gap-4">
                    <button 
                      type="submit"
                      disabled={creating}
                      className="flex-1 bg-primary hover:bg-neutral-200 disabled:bg-neutral-800 disabled:text-neutral-600 text-black py-4 rounded-xl font-black text-[11px] uppercase tracking-widest shadow-lg transition-all border border-white/10 flex items-center justify-center gap-2 italic"
                    >
                      {creating ? (
                    ...
                    <button 
                      type="button"
                      onClick={() => setIsCreateModalOpen(false)}
                      className="px-8 py-4 text-neutral-600 hover:text-white transition-colors text-[11px] font-black uppercase tracking-widest italic"
                    >
                      Cancel
                    </button>
                  </div>
