  ...
  return (
    <motion.aside
      ...
      animate={{ 
        ...
        x: isMobileOpen ? 0 : (window.innerWidth < 1024 ? -256 : 0)
      }}
      className={`h-screen fixed left-0 top-0 bg-[#050505] border-r border-white/[0.04] flex flex-col z-[60] lg:z-50 overflow-hidden transition-transform duration-300 lg:translate-x-0 ${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}
    >
      {/* Brand */}
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-2">
            {!isCollapsed && (
              <button
                onClick={onToggle}
                className="hidden lg:block p-1.5 rounded-lg bg-white/[0.03] border border-white/[0.06] text-neutral-700 hover:text-primary hover:bg-white/[0.05] transition-all"
              >
            ...
            <button
              ...
              className="lg:hidden p-2 rounded-xl bg-white/[0.03] border border-white/[0.06] text-neutral-500 hover:text-white transition-all"
            >
               <HugeiconsIcon icon={SidebarRight01Icon} size={18} className="rotate-180" variant="stroke" strokeWidth={1.5} />
            </button>
          </div>
        </div>
            ...
            <div key={section.group} className="mb-1">
              ...
              <AnimatePresence initial={false}>
                {(isExpanded || isCollapsed || isMobileOpen) && (
                  <motion.div
                    initial={(!isCollapsed || isMobileOpen) ? { height: 0, opacity: 0 } : false}
                    animate={{ height: "auto", opacity: 1 }}
                    exit={(!isCollapsed || isMobileOpen) ? { height: 0, opacity: 0 } : {}}
                    transition={{ duration: 0.2, ease: "easeInOut" }}
                    className={`overflow-hidden space-y-0.5 ${(isCollapsed && !isMobileOpen) ? "flex flex-col items-center" : ""}`}
                  >
                    {section.items.map((item: any) => {
                      ...
                      return (
                        <Link
                          ...
                          to={item.href}
                          onClick={onMobileClose}
                          title={(isCollapsed && !isMobileOpen) ? `${item.name}${item.note ? ` — ${item.note}` : ""}` : ""}
                          className={`
                            flex items-center transition-all duration-200 group relative
                            ${(isCollapsed && !isMobileOpen) ? "justify-center w-11 h-11 rounded-xl mx-auto" : "gap-3 px-3 py-2.5 rounded-xl"}
                            ${isActive
                        ...
                        >
                          ...
                          <div className={`relative flex-shrink-0 transition-all duration-200 ${isActive ? "text-primary" : isHintItem ? "text-primary/40" : "text-neutral-600 group-hover:text-neutral-400"}`}>
                            <HugeiconsIcon icon={Icon} size={(isCollapsed && !isMobileOpen) ? 18 : 15} variant="stroke" strokeWidth={isActive ? 2 : 1.5} />
                            {isActive && (
                          ...
                          {!isActive && (!isCollapsed || isMobileOpen) && !isHintItem && (
